...
describe("listSharedExamSetsService", () => {
      ...
      Promise.resolve([
        {
          items: [
            {
              share: { _id: "707f...", permission: "VIEW", status: "ACTIVE", effectiveStatus: "ACTIVE", expiresAt: null, note: "", sharedAt: new Date("2026-01-01T00:00:00Z"), createdAt: new Date("2026-01-01T00:00:00Z"), updatedAt: new Date("2026-01-01T00:00:00Z") },
              examSet: { _id: "607f...", title: "Title X", description: "Desc", tags: ["math"], status: "draft", metrics: { totalQuestions: 10, totalPoints: 20 }, versionNumber: 1, rootExamSetId: "607f...", isLatestVersion: true, createdAt: new Date("2026-01-01T00:00:00Z"), updatedAt: new Date("2026-01-01T00:00:00Z") },
  ...
  it("Teacher gets shared exam set with EDIT permission", async () => {
    vi.spyOn(ExamSetShare, "aggregate").mockImplementation(() =>
      Promise.resolve([{ items: [{ share: { permission: "EDIT" }, examSet: { title: "Title Y" }, owner: { email: "owner@example.com" } }], totalCount: [{ count: 1 }] }])
    );
  ...
  it("Empty list returns 200 with pagination totalItems 0", async () => {
    vi.spyOn(ExamSetShare, "aggregate").mockImplementation(() => Promise.resolve([{ items: [], totalCount: [] }]));
