...
class AIQuestionGenerationService {
  ...
  async generateQuestionSet(lesson, userId, userRole, requestConfig) {
    ...
    if (!folder) {
      throw new AIError("Folder không tồn tại hoặc bạn không có quyền truy cập", AIErrorCode.AI_INVALID_INPUT, 404);
    }

    // 1. Extract lesson content
    const { text: lessonText, warnings: sourceWarnings } = await lessonContentExtractor.extractLessonContent(lesson);
    ...
    if (existingExamSet) {
      throw new AIError("Yêu cầu sinh câu hỏi bị trùng lặp (Idempotency). Bộ đề đã được sinh ra trước đó.", AIErrorCode.AI_INVALID_INPUT, 409);
    }

    // 5. Call AI Core orchestration
    // We wrap validator to inject requestConfig
    const customValidator = (parsedData) => validateQuestionGenerationOutput(parsedData, requestConfig);
