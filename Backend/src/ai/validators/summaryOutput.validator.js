...
export const summaryOutputValidator = (data) => {
  if (!data || typeof data !== "object") {
    throw new AIError(
      "Output AI trả về không phải là JSON object hợp lệ.",
  ...
  if (keyPoints.length > 20) {
    throw new AIError(
      "keyPoints vượt quá số lượng cho phép (tối đa 20).",
      AIErrorCode.AI_OUTPUT_INVALID,
      502
    );
  }
  ...
  if (suggestedReviewTopics !== undefined && !Array.isArray(suggestedReviewTopics)) {
    throw new AIError(
      "suggestedReviewTopics không phải là mảng.",
      AIErrorCode.AI_OUTPUT_INVALID,
      502
    );
  }

  suggestedReviewTopics = (suggestedReviewTopics || [])
    .filter((topic) => typeof topic === "string" && topic.trim().length > 0)
    .map((topic) => topic.trim());

  if (suggestedReviewTopics.length > 15) {
    throw new AIError(
      "suggestedReviewTopics vượt quá số lượng cho phép (tối đa 15).",
      AIErrorCode.AI_OUTPUT_INVALID,
      502
    );
  }
