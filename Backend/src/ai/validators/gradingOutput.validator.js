...
export const validateGradingOutput = (rawResponse, maxScore) => {
  ...
  if (!parsedData || typeof parsedData !== "object") {
    throw new AIError(
      "Dữ liệu chấm điểm từ AI không hợp lệ",
  ...
  if (typeof parsedData.suggestedScore !== "number" || !Number.isFinite(parsedData.suggestedScore)) {
  ...
  if (parsedData.confidence < 0 || parsedData.confidence > 1) {
    throw new AIError("confidence phải nằm trong khoảng [0, 1]", AIErrorCode.AI_OUTPUT_INVALID, 502);
  }
