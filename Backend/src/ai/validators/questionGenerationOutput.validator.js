...
export const validateQuestionGenerationOutput = (parsedData, requestConfig) => {
  ...
  if (questions.length !== questionCount) {
    throw new AIError(`AI sinh sai số lượng câu hỏi. Yêu cầu: ${questionCount}, Thực tế: ${questions.length}`, AIErrorCode.AI_OUTPUT_INVALID, 422);
  }
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    if (!actualTypes.hasOwnProperty(type)) {
      throw new AIError(`Câu ${i + 1}: Type không hỗ trợ '${type}'`, AIErrorCode.AI_OUTPUT_INVALID, 422);
    }
    if (!actualDifficulties.hasOwnProperty(difficulty)) {
      throw new AIError(`Câu ${i + 1}: Difficulty không hỗ trợ '${difficulty}'`, AIErrorCode.AI_OUTPUT_INVALID, 422);
    }
    ...
    if (type === "multiple_choice" || type === "true_false") {
      ...
      if (type === "multiple_choice" && q.options.length < 2) {
        throw new AIError(`Câu ${i + 1}: Multiple choice cần ít nhất 2 options`, AIErrorCode.AI_OUTPUT_INVALID, 422);
      }
      if (type === "true_false" && q.options.length !== 2) {
        throw new AIError(`Câu ${i + 1}: True/False phải có đúng 2 options`, AIErrorCode.AI_OUTPUT_INVALID, 422);
      }

      const optIds = new Set();
      validatedQ.options = q.options.map(opt => {
        if (!opt.id || !opt.text || typeof opt.text !== "string") {
          throw new AIError(`Câu ${i + 1}: Option thiếu id hoặc text`, AIErrorCode.AI_OUTPUT_INVALID, 422);
        }
        const optId = String(opt.id).trim();
        if (optIds.has(optId)) {
          throw new AIError(`Câu ${i + 1}: Option id '${optId}' bị trùng`, AIErrorCode.AI_OUTPUT_INVALID, 422);
        }
        optIds.add(optId);
        return {
          id: optId,
      ...
      if (!optIds.has(ansId)) {
        throw new AIError(`Câu ${i + 1}: correctAnswer '${ansId}' không nằm trong options`, AIErrorCode.AI_OUTPUT_INVALID, 422);
      }
      ...
      if (!q.correctAnswer || typeof q.correctAnswer !== "string") {
        throw new AIError(`Câu ${i + 1}: Thiếu correctAnswer cho short_answer`, AIErrorCode.AI_OUTPUT_INVALID, 422);
      }
  ...
  for (const [t, count] of Object.entries(questionTypes || {})) {
    if (actualTypes[t] !== count) {
      throw new AIError(`Phân bố type '${t}' sai. Yêu cầu: ${count}, Thực tế: ${actualTypes[t]}`, AIErrorCode.AI_OUTPUT_INVALID, 422);
    }
  }

  for (const [d, count] of Object.entries(difficultyDistribution || {})) {
    if (actualDifficulties[d] !== count) {
      throw new AIError(`Phân bố difficulty '${d}' sai. Yêu cầu: ${count}, Thực tế: ${actualDifficulties[d]}`, AIErrorCode.AI_OUTPUT_INVALID, 422);
    }
  }
