...
const mockExamSet = {
  ...
  questions: [
    ...
    { questionId: "q5", type: "true_false", difficulty: "easy", options: [{ id: "o2", text: "True", isCorrect: true }] },
    { questionId: "q6", type: "true_false", difficulty: "medium", options: [{ id: "o3", text: "False", isCorrect: true }] },
...
async function runUnitTests() {
  ...
  const resetMocks = () => {
    ...
    classModel.findOne = async () => ({ _id: fakeClassId, teacherId: fakeTeacherId, isDeleted: false });
    classModel.findById = () => ({
      lean: async () => ({ _id: fakeClassId, teacherId: fakeTeacherId, students: [{ studentId: fakeStudentId }] })
    });
    ...
    Exam.prototype.save = async function() { return this; };
  };
  ...
  await runTest("Validation: totalPoints = 9 trả 400", async () => {
    const req = { user: { _id: fakeTeacherId, role: "teacher" }, body: { classId: fakeClassId, examSetId: fakeExamSetId, title: "T", durationMinutes: 10, totalQuestions: 1, totalPoints: 9, questionTypeDistribution: {}, difficultyDistribution: {} } };
    const res = createMockRes();
    await generateFromExamSet(req, res);
    assert.strictEqual(res.statusCode, 400);
  ...
  await runTest("Validation: totalPoints = 11 trả 400", async () => {
    const req = { user: { _id: fakeTeacherId, role: "teacher" }, body: { classId: fakeClassId, examSetId: fakeExamSetId, title: "T", durationMinutes: 10, totalQuestions: 1, totalPoints: 11, questionTypeDistribution: {}, difficultyDistribution: {} } };
    const res = createMockRes();
    await generateFromExamSet(req, res);
    assert.strictEqual(res.statusCode, 400);
    assert.ok(res.data.message.includes("10"));
  ...
  await runTest("Validation: NaN totalPoints trả 400", async () => {
    const req = { user: { _id: fakeTeacherId, role: "teacher" }, body: { classId: fakeClassId, examSetId: fakeExamSetId, title: "T", durationMinutes: 10, totalQuestions: 1, totalPoints: NaN, questionTypeDistribution: {}, difficultyDistribution: {} } };
    const res = createMockRes();
    await generateFromExamSet(req, res);
    assert.strictEqual(res.statusCode, 400);
  });
  ...
  await runTest("Validation: totalPoints = 10, config hợp lệ trả 201", async () => {
    const req = { user: { _id: fakeTeacherId, role: "teacher" }, body: { classId: fakeClassId, examSetId: fakeExamSetId, title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 } } };
    const res = createMockRes();
    await generateFromExamSet(req, res);
  ...
  await runTest("Fingerprint: Đổi thứ tự key không đổi fingerprint (Canonicalize)", async () => {
    const bp1 = { title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 }, a: 1, b: 2 };
    const bp2 = { title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 }, b: 2, a: 1 };
    ...
    await aiExamGenerationService.generateExamFromSet({ userId: fakeTeacherId, classId: fakeClassId, examSetId: fakeExamSetId, blueprint: bp1 });
    await aiExamGenerationService.generateExamFromSet({ userId: fakeTeacherId, classId: fakeClassId, examSetId: fakeExamSetId, blueprint: bp2 });
  ...
  await runTest("Fingerprint: Config khác nhau tạo fingerprint khác nhau", async () => {
    ...
    const bp1 = { title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 }, a: 1 };
    const bp2 = { title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 }, a: 2 };
  ...
  await runTest("Idempotency: Trả 409 nếu request lặp lại (cùng fingerprint và draft tồn tại)", async () => {
    resetMocks();
    Exam.findOne = async () => ({ _id: "existing_draft" });
    try {
      await aiExamGenerationService.generateExamFromSet({ userId: fakeTeacherId, classId: fakeClassId, examSetId: fakeExamSetId, blueprint: { totalQuestions: 1, totalPoints: 10 } });
  ...
  await runTest("Class IDOR: Giáo viên không phải chủ lớp bị chặn", async () => {
    resetMocks();
    classModel.findOne = async () => ({ _id: fakeClassId, teacherId: "another_teacher", isDeleted: false });
    const req = { user: { _id: fakeTeacherId, role: "teacher" }, body: { classId: fakeClassId, examSetId: fakeExamSetId, title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 } } };
    const res = createMockRes();
    await generateFromExamSet(req, res);
  ...
  await runTest("ExamSet IDOR: Quyền Admin bypass ExamSet owner", async () => {
    resetMocks();
    ExamSet.findOne = () => ({ lean: async () => ({ ...mockExamSet, ownerId: "another_owner" }) });
    const req = { user: { _id: fakeAdminId, role: "admin" }, body: { classId: fakeClassId, examSetId: fakeExamSetId, title: "T", durationMinutes: 10, totalQuestions: 2, totalPoints: 10, questionTypeDistribution: { essay: 2 }, difficultyDistribution: { hard: 2 } } };
    const res = createMockRes();
    await generateFromExamSet(req, res);
  ...
  const mockDraftExam = {
    _id: fakeExamId, classId: fakeClassId, status: "DRAFT", createdBy: fakeTeacherId,
    questions: [
      { isSnapshot: true, snapshotData: { id: "q1", correctAnswer: "A", options: [{ isCorrect: true, text: "A" }] } }
    ]
  };
  const mockPubExam = {
    _id: fakeExamId, classId: fakeClassId, status: "PUBLISHED", createdBy: fakeTeacherId,
    questions: [
      { isSnapshot: true, snapshotData: { id: "q1", correctAnswer: "A", rubric: "R", options: [{ isCorrect: true, text: "A" }] } }
    ]
  };
  ...
  await runTest("Student View: Lớp khác bị chặn", async () => {
    ...
    classModel.findById = () => ({ lean: async () => ({ _id: fakeClassId, students: [{ studentId: "another" }] }) });
    const req = { params: { id: fakeExamId }, user: { _id: fakeStudentId, role: "student" } };
    const res = createMockRes();
    await getExamById(req, res);
    assert.strictEqual(res.statusCode, 404);
  ...
  await runTest("Student View: Đúng lớp xem được PUBLISHED, bị ẩn correctAnswer/rubric/isCorrect", async () => {
    resetMocks();
    Exam.findById = () => ({ populate: () => ({ lean: async () => mockPubExam }) });
    const req = { params: { id: fakeExamId }, user: { _id: fakeStudentId, role: "student" } };
    const res = createMockRes();
  ...
  await runTest("Draft Lifecycle: DRAFT hết thời gian không chuyển COMPLETED", async () => {
    ...
    Exam.find = () => ({
      sort: () => [
        { status: "DRAFT", startTime: pastTime, duration: 1, save: async () => { saved = true; } },
        { status: "PUBLISHED", startTime: pastTime, duration: 1, save: async () => { saved = true; } }
      ]
    });
    const req = {};
    const res = createMockRes();
