...
export const updateClassValidation = [
  body("className").optional().isString().trim().isLength({ min: 3 }).withMessage("Tên lớp học tối thiểu 3 ký tự"),
  body("classCode").optional().isString().trim(),
  body("classRoom").optional().isString().trim(),
  body("learningMode").optional().isIn(["Offline", "Online", "Hybrid"]).withMessage("learningMode không hợp lệ"),
  ...
  body("maxStudents").optional().isInt({ min: 1 }).withMessage("maxStudents phải là số nguyên >= 1"),
