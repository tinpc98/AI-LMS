...
const parseOptionsAndCorrectAnswer = (rawOptions, rawCorrectAnswer, type, rowNum) => {
  ...
  if (type === "multiple_choice") {
    if (!rawOptions) {
      throw new Error(`Dòng ${rowNum}: Câu hỏi trắc nghiệm (multiple_choice) bắt buộc phải có options`);
    }
    const optionTexts = String(rawOptions).split("|").map(s => s.trim()).filter(s => s);
    ...
    if (!correctId) {
      throw new Error(`Dòng ${rowNum}: Đáp án đúng (correctAnswer: ${caStr}) không khớp với bất kỳ option nào trong danh sách`);
    }
    ...
    if (!rawCorrectAnswer && rawCorrectAnswer !== 0) {
      throw new Error(`Dòng ${rowNum}: Câu hỏi điền khuyết (short_answer) bắt buộc phải có correctAnswer`);
    }
