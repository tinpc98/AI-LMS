    ...
    <Drawer title="Course Details" placement="right" onClose={onClose} open={open} width={460}>
      {course ? (
        <Descriptions column={1} bordered>
          ...
          <Descriptions.Item label="Tuition Fee">{course.tuitionFee.toLocaleString()} VND</Descriptions.Item>
          ...
          <Descriptions.Item label="Created At">{new Date(course.createdAt).toLocaleString("en-GB")}</Descriptions.Item>
          <Descriptions.Item label="Updated At">{new Date(course.updatedAt).toLocaleString("en-GB")}</Descriptions.Item>
        </Descriptions>
      ) : null}
    </Drawer>
  );
