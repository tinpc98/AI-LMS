...
export const AssignmentOverviewWidget: React.FC<AssignmentOverviewWidgetProps> = React.memo(({ assignments }) => {
  return (
    <Card
      title={
        <span style={{ fontSize: 15, fontWeight: 700 }}>
          <FileTextOutlined style={{ color: "#1890ff", marginRight: 6 }} /> Tổng quan Bài tập ({assignments.length})
        </span>
      }
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  ...
                  {isSubmitted ? (
                    ...
                    <Tag color="warning" icon={<ClockCircleOutlined />} style={{ borderRadius: 6 }}>
                      Chưa nộp
                    </Tag>
                  )}
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    Lớp: {item.className} • Hạn nộp: {new Date(item.dueDate).toLocaleDateString("vi-VN")}
                  </Text>
