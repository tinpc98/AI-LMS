...
export const TeacherClassListTable: React.FC<TeacherClassListTableProps> = React.memo(({ classes, loading }) => {
  ...
  const columns: ColumnsType<ClassItem> = [
    ...
    },
    {
      ...
      key: "students",
      render: (_, record) => {
        const count = record.currentStudents ?? (Array.isArray(record.students) ? record.students.length : 0);
  ...
  return (
    <Table
      ...
      loading={loading}
      pagination={false}
      style={{ backgroundColor: "#fff", borderRadius: 12, overflow: "hidden", border: "1px solid #f0f0f0" }}
    />
  );
});
