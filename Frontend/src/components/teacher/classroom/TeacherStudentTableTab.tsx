...
export const TeacherStudentTableTab: React.FC<TeacherStudentTableTabProps> = React.memo(({ students = [], loading = false }) => {
  ...
  const columns: ColumnsType<StudentItem> = [
    ...
    },
    {
      ...
      dataIndex: "joinedAt",
      key: "joinedAt",
      render: (joinedAt) => (joinedAt ? new Date(joinedAt).toLocaleDateString("vi-VN") : "Đã vào lớp"),
    },
    {
      title: "Trạng thái",
      dataIndex: "status",
        ...
        locale={{
          emptyText: (
            <Empty
              ...
              description={searchQuery ? "Không tìm thấy học sinh nào phù hợp từ khóa!" : "Lớp học chưa có học sinh nào."}
            />
          ),
        }}
      />
