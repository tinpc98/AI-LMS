...
interface TeacherInformationCardProps {
  teacher?: {
    ...
    availabilitySchedule?: Record<string, { startTime?: string; endTime?: string; available?: boolean }> | null;
  } | null;
}

export const TeacherInformationCard: React.FC<TeacherInformationCardProps> = React.memo(({ teacher }) => {
  if (!teacher) {
    return (
      <Card
        title={
      ...
      >
        <EmptyState description="Lớp học hiện chưa có thông tin phân công giảng viên." style={{ border: "none" }} />
      </Card>
    );
  }

  const subjects = teacher.teachingSubjects && teacher.teachingSubjects.length > 0
    ? teacher.teachingSubjects
    : ["Công nghệ thông tin", "Khoa học máy tính"];

  const hasSchedule = teacher.availabilitySchedule && Object.keys(teacher.availabilitySchedule).length > 0;
        ...
        <div>
          ...
          <div style={{ backgroundColor: "#fafafa", borderRadius: 8, padding: "8px 12px" }}>
            {Object.entries(teacher.availabilitySchedule!).map(([day, info]) => {
              ...
              return (
                <div key={day} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, margin: "2px 0" }}>
                  <Text strong>{day}:</Text>
                  <Text type="secondary">{info.startTime} - {info.endTime}</Text>
                </div>
              );
            })}
          </div>
