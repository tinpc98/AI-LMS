...
export const ReportOverview: React.FC = () => {
  ...
  const classStatusData = [
    { name: "Đang diễn ra (Active)", value: mockClasses.filter((c) => c.status === "Active").length },
    { name: "Sắp mở (Upcoming)", value: mockClasses.filter((c) => c.status === "Upcoming").length },
    { name: "Đã hoàn thành (Completed)", value: mockClasses.filter((c) => c.status === "Completed").length },
  ];

  const aiFeatureData = [
    { feature: "Chatbot AI", usage: mockAILogs.filter((l) => l.feature === "Chatbot").length * 45 + 120 },
    { feature: "Tóm tắt bài học", usage: mockAILogs.filter((l) => l.feature === "Summary").length * 35 + 85 },
    { feature: "Tạo trắc nghiệm", usage: mockAILogs.filter((l) => l.feature === "Quiz").length * 28 + 60 },
    { feature: "Chấm điểm tự động", usage: mockAILogs.filter((l) => l.feature === "Exam").length * 19 + 40 },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <Statistic
              ...
              suffix={<Tag color="blue" className="ml-2 rounded-full"><RiseOutlined /> +14.2%</Tag>}
            />
            <div className="mt-3 text-xs text-gray-400">Đang hoạt động: {students.length} tài khoản mẫu</div>
          </Card>
        </Col>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <Statistic
              ...
              suffix={<Tag color="purple" className="ml-2 rounded-full"><RiseOutlined /> +5.0%</Tag>}
            />
            <div className="mt-3 text-xs text-gray-400">Giáo viên active: {teachers.filter(t => t.status === "Active").length}</div>
          </Card>
        </Col>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <Statistic
              ...
              value={totalClasses}
              prefix={<BookOutlined className="text-emerald-500 mr-2 p-2 bg-emerald-50 rounded-lg" />}
              suffix={<Tag color="emerald" className="ml-2 rounded-full"><CheckCircleOutlined /> {activeClasses} Đang mở</Tag>}
            />
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <Statistic
              ...
              suffix={<Tag color="amber" className="ml-2 rounded-full"><RiseOutlined /> +28%</Tag>}
            />
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12}>
          <Card title="Tỷ lệ lấp đầy lớp học" className="rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-2">
              <span className="text-gray-600 font-medium">Học sinh đăng ký / Tổng chỗ</span>
              <span className="font-bold text-blue-600">{totalEnrolled} / {totalCapacity} ({fillRate}%)</span>
            </div>
            <Progress percent={fillRate} status="active" strokeColor={{ '0%': '#108ee9', '100%': '#87d068' }} />
          </Card>
        </Col>
        <Col xs={24} md={12}>
          <Card title="Phòng học trực tuyến Live Session" className="rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <VideoCameraOutlined className="text-2xl text-red-500 p-3 bg-red-50 rounded-xl" />
                <div>
                  <div className="text-lg font-bold text-gray-800">{liveSessionsCount} Phòng học đang hoạt động</div>
                  <div className="text-xs text-gray-500">Nền tảng tích hợp Jitsi & Zoom SDK</div>
                </div>
              </div>
              <Tag color="red" className="px-3 py-1 text-xs font-semibold rounded-full animate-pulse">● LIVE NOW</Tag>
            </div>
          </Card>
        </Col>
      </Row>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyTrendData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Area type="monotone" dataKey="students" name="Học sinh" stroke="#1677ff" fillOpacity={1} fill="url(#colorStudents)" />
                  <Area type="monotone" dataKey="aiUsage" name="Lượt dùng AI" stroke="#722ed1" fillOpacity={1} fill="url(#colorAI)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </Card>
